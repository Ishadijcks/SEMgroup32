...
public class Collisions implements Observable {
    ...
    public void allCollisions(Game game) {
        ...
        checkCollisionPlayer(game.getCurrentLevel().getBubbleList(), game
                .getCurrentLevel().getPlayerList());
        checkPlayerCollisionWall(game.getCurrentLevel().getWallList(), game
                .getCurrentLevel().getPlayerList());
        checkBubbleCollisionWall(game.getCurrentLevel().getWallList(), game
                .getCurrentLevel().getBubbleList());
    }
     ...
     */
    public int checkCollisionRope(ArrayList<Bubble> bubbleList, Rope rope) {
        for (int i = 0; i < bubbleList.size(); i++) {
            if (bubbleList.get(i).getX() <= rope.getX()) {
                if (bubbleList.get(i).getX() + bubbleList.get(i).getDiameter() >= rope
                        .getX()) {
     ...
     * @param powerupList
     *            List of powerups
     * @return The powerup that collided will be returned, if there are no
     *         collisions then it will return -1
     */
    public int checkCollisionPowerup(ArrayList<Player> playerList,
